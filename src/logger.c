...
static volatile int logging_active = 0;
static volatile char message_buffer[MESSAGE_BUFFER_SIZE];
...
static const unsigned const ee_metadata_ipee_api_e_api_values[] = {
...
static const unsigned const ee_metadata_ipee_api_e_log_level_values[] = {
    LEVEL_DEBUG,
    LEVEL_INFO,
    LEVEL_NOTICE,
    LEVEL_WARNING,
...
void logger_clear_data_from_log_file(void) {
    ...
    if (logger_file.log_file && logger_file.log_file != stdout) {
        ...
        logger_file.log_file = freopen(logger_file.log_file_name, "a", logger_file.log_file);
    }
...
void internal_log(unsigned api, log_level_t level, const char *f, unsigned l, const char *format, ...) {
    ...
    if (n >= 0) {
        ...
        sprintf(message_buffer, "%04d-%02d-%02d %02d:%02d:%02d.%d [%s] %s:%s:%d %s\n",
