 ...
 */
typedef struct __impl_container_s {
...
} __impl_container_t, *__p_impl_container;
 ...
 */
static void release_service(char *key,
    container_callback_function callback, __p_impl_container container);
...
p_container init_container(const char *name) {
    ...
    __p_impl_container impl_container = (__p_impl_container)malloc(sizeof(__impl_container_t));
    if (!impl_container)
        return NULL;

    impl_container->name = name;
    impl_container->services = create_dictionary();
    impl_container->elements_types = create_dictionary();
    impl_container->elements_initial_callback = create_dictionary();
    impl_container->elements_release_callback = create_dictionary();
    impl_container->elements_args = create_dictionary();
    impl_container->elements_refs = create_dictionary();
    pthread_mutex_init(&impl_container->mutex, NULL);

    add_record_to_dictionary(containers, name, impl_container);

    return (p_container)impl_container;
}
...
void release_container(p_container container) {
    ...
    __p_impl_container impl_container = (__p_impl_container)container;
    pthread_mutex_lock(&impl_container->mutex);
    if (impl_container) {
        iterate_over_dictionary_with_args(impl_container->elements_release_callback, (void *)release_service, impl_container);
        delete_dictionary(impl_container->services);
        impl_container->services = NULL;
        delete_dictionary(impl_container->elements_types);
        impl_container->elements_types = NULL;
        delete_dictionary(impl_container->elements_initial_callback);
        impl_container->elements_initial_callback = NULL;
        delete_dictionary(impl_container->elements_release_callback);
        impl_container->elements_release_callback = NULL;
        delete_dictionary(impl_container->elements_args);
        impl_container->elements_args = NULL;
        delete_dictionary(impl_container->elements_refs);
        impl_container->elements_refs = NULL;
    }
    pthread_mutex_unlock(&impl_container->mutex);
    pthread_mutex_destroy(&impl_container->mutex);
...
void add_service_to_container(
    ...
    pthread_mutex_lock(&impl_container->mutex);
    if (impl_container) {
        container_callback_function callback = get_value_from_dictionary(impl_container->elements_release_callback, key);
        if (callback)
            release_service(key, callback, container);

        switch (type) {
        case SERVICE_TYPE_SINGLETON:
            add_record_to_dictionary(impl_container->services, impl_container->services->size, key);
            add_record_to_dictionary(impl_container->elements_types, key, (void *)SERVICE_TYPE_SINGLETON);
            add_record_to_dictionary(impl_container->elements_initial_callback, key, initial_callback);
            add_record_to_dictionary(impl_container->elements_release_callback, key, release_callback);
            add_record_to_dictionary(impl_container->elements_refs, key, NULL);
            add_record_to_dictionary(impl_container->elements_args, key, args);
            break;

        case SERVICE_TYPE_TRANSIENT:
            add_record_to_dictionary(impl_container->services, impl_container->services->size, key);
            add_record_to_dictionary(impl_container->elements_types, key, (void *)SERVICE_TYPE_TRANSIENT);
            add_record_to_dictionary(impl_container->elements_initial_callback, key, initial_callback);
            add_record_to_dictionary(impl_container->elements_release_callback, key, release_callback);
            add_record_to_dictionary(impl_container->elements_refs, key, create_dictionary());
            add_record_to_dictionary(impl_container->elements_args, key, args);
            break;

        case SERVICE_TYPE_GLBLVALUE:
            add_record_to_dictionary(impl_container->services, impl_container->services->size, key);
            add_record_to_dictionary(impl_container->elements_types, key, (void *)SERVICE_TYPE_GLBLVALUE);
            add_record_to_dictionary(impl_container->elements_initial_callback, key, NULL);
            add_record_to_dictionary(impl_container->elements_release_callback, key, release_callback);
            add_record_to_dictionary(impl_container->elements_args, key, args);
            break;
...
void *get_service_from_container_with_args(
    ...
    service_type_t type = (service_type_t)get_value_from_dictionary(
        impl_container->elements_types, key);
    switch (type) {
    case SERVICE_TYPE_SINGLETON:
        void *instance = get_value_from_dictionary(impl_container->elements_refs, key);
        if (!instance) {
            container_callback_function initial_callback = get_value_from_dictionary(
                    impl_container->elements_initial_callback, key);
            void *args = get_value_from_dictionary(impl_container->elements_args, key);
            if (tmp_args)
                args = tmp_args;

            instance = initial_callback(args);
            update_record_in_dictionary(impl_container->elements_refs, key, instance);
        }
    ...
    case SERVICE_TYPE_TRANSIENT:
        ...
        p_dictionary refs =
            get_value_from_dictionary(impl_container->elements_refs, key);
        if (refs->size >= MAX_TRANSIENT_REFS) {
            pthread_mutex_lock(&impl_container->mutex);
            if (refs->size) {
                void *head_value = get_head_value_from_dictionary(refs);
                container_callback_function release_callback = 
                    get_value_from_dictionary(impl_container->elements_release_callback, key);
        ...
        container_callback_function initial_callback =
            get_value_from_dictionary(impl_container->elements_initial_callback, key);
        add_record_to_dictionary(refs, key, initial_callback(args));
        return get_tail_value_from_dictionary(refs);

    case SERVICE_TYPE_GLBLVALUE:
        return get_value_from_dictionary(impl_container->elements_args, key);
...
static void release_service(
    char *key, container_callback_function callback, __p_impl_container container) {
    if (!containers)
        exit(IPEE_ERROR_CODE__CONTAINER__SERVICE_UNINITIALIZED);
    if (!container)
        exit(IPEE_ERROR_CODE__CONTAINER__NOT_EXISTS);
