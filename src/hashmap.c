...
typedef struct hashmap_s {
    ...
    hashmap_malloc_entry_callback entry_malloc; // Callback function to allocate memory for entry
    hashmap_free_entry_callback entry_free;     // Callback function to free memory for entry
} hashmap_t, *p_hashmap;
...
p_hashmap hashmap_create_for_dynamic_entry(hashmap_malloc_entry_callback entry_malloc,
                                           hashmap_free_entry_callback entry_free) {
    if ((entry_malloc && !entry_free) ||
        (entry_free && !entry_malloc)) {
        return NULL;
    }
    ...
    map->entry_malloc = entry_malloc;
    map->entry_free = entry_free;

    return map;
}

p_hashmap hashmap_create(void) {
    return hashmap_create_for_dynamic_entry(NULL, NULL);
}
...
void hashmap_set_entry(p_hashmap map, p_key *key, size_t ksize, void *value) {
    ...
    uint32_t hash = hashmap_hash_default(*key, ksize);
...
void hashmap_remove_entry(p_hashmap map, p_key key, size_t ksize) {
    ...
    if (entry->key) {
        ...
        if (map->entry_free && entry->value) {
            map->entry_free(&entry->value);
        }
...
void hashmap_remove_all_entries(p_hashmap map) {
    ...
    while (current) {
        if (current->key) {
            ...
            if (map->entry_free && current->value) {
                map->entry_free(&current->value);
            }
        }
